...
export default function Header() {
  ...
  return ( 
    ...
    <div className={`w-full  ${isScrolled ? 'bg-gray-100/95 backdrop-blur shadow-ml py-2' : 'bg-gray-100 py-4'}`}>
    ...
    <button className='relative p-2 text-gray-700 hover:pink-600'
            aria-label='wishlist'>
            <FiHeart size={20}/>
    ...
    {navItems.map((item)=>(
        <li key={item.id}>
            <a href={item.link} className='hover:text-pink-300 transition-color'>
                {item.name}
            </a>
        </li>
    ))}
